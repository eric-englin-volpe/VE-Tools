# Extract VERSPM results to join with equity analysis
...
# Extract and compile information to Bzone,  Marea and Household Level CSV file
modelname <- 'VERSPM_VDOT'
cat("\n\n Extracting metrics from", modelname, '\n')
modelpath <- file.path(ve.runtime, 'models', modelname)
# 
...
results_base <- extract_scenario_metrics(modelname, modelpath, baseyear)

marea <- results_base[[1]] # Marea
hh    <- results_base[[2]] # Household


write.csv(marea, file.path(modelpath, 'Metrics_Marea.csv'),
        row.names = F)
write.csv(hh,  file.path(modelpath, 'Metrics_Hh.csv'),
        row.names = F)
...
dfls <- get(load(file.path(modelpath, "Datastore", "DatastoreListing.Rda")))
...
write.csv(metric_units, file.path(modelpath, "Extracted_Metric_Units.csv"),
          row.names = F)

# Save as RData for faster loading in R (compared to loading individual .csv files)

save(file = file.path(modelpath, 'Single_Run_Complete.RData'),
     list = c('metric_units',
              'hh',
              'marea',
              'atts'))
